...
export default function TeacherSection() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await api.put(`${API_URL}/teachers/${editingId}`, body);
      } else {
        await api.post(`${API_URL}/teachers`, body);
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await api.delete(`${API_URL}/teachers/${deleteId}`);
      toast('Teacher deleted', '', { label: 'Undo', onClick: async () => {
        try {
          await api.post(`${API_URL}/teachers/${idToRestore}/restore`);
          ...
          <button
            onClick={async () => {
              ...
              await Promise.all(filtered.filter((t: any) => t.photoUrl).map(async (t: any) => {
                try { const r = await api.get(t.photoUrl, { responseType: 'blob' }); photoCache[t.id] = await new Promise<string>(res => { const reader = new FileReader(); reader.onload = () => res(reader.result as string); reader.readAsDataURL(r.data); }); } catch {}
              }));
              const doc = new jsPDF();
